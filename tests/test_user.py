...
def test_if_user_has_all_attributes():
    ...
    email_regex = r'^[\w\.-]+@[\w\.-]+\.\w+$'
    assert re.match(email_regex, user.email), f"Email '{user.email}' does not match the expected format."

    phone_number_regex = r'^\+\d{1,3}\s\d{3,}$'
    assert re.match(phone_number_regex, user.phone_number), f"Phone number '{user.phone_number}' does not match the expected format."
